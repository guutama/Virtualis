...
def plot_prediction(y_true, y_pred, inputs=None, info="", backend="plotly"):
    ...
    ax1.set_xlabel("time ")
    ax1.set_ylabel("X1_ActualPosition (mm)")
    ...
    if inputs is not None:
        ...
        ax2 = ax1.twinx()
        ax2.set_ylabel("X1_ActualPosition_with_missing_values (mm)")

        for i in range(num_features):
            pass
            #ax2.plot(inputs[:, -1, i], label=input_columns.iloc[i + 1, 1])
            #ax1.plot(inputs[:, -1, i], label=input_columns.iloc[i + 1, 1])
            """added above line for temporary test"""
    fig.legend()
    ...
    if backend == "matplotlib":
        ...
        fig.update_yaxes(title_text="X1_ActualPosition (mm)",
                         secondary_y=False)
        # fig.update_yaxes(
        #     title_text="X1_ActualPosition_with_missing_values (mm)", secondary_y=True)
